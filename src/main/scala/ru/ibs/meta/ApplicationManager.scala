...
class ApplicationManager(input: Map[String, String])(implicit app: Application) {
  ...
  val source: DataStorage = getConnector(input, "source")

  val target: DataStorage = getConnector(input, "target")
}
